...
export function getSortedDataByDirections(id) {
    ...
    const json = filenames
          .filter(filename => filename === 'directions.json')
          .map(filename => fs.readFileSync(path.join(directionsDirectory, filename), 'utf8'));
    ...
    const ar = [...j].flatMap(({ licenses, name }) => licenses
                              .map(({ university_edrpou,
                                      university_name,
                                      university_financing_type_name,
                                      region_name,
...
export function getSortedDirectionsData() {
    return directions[0].sort((a, b) => (a.category_id - b.category_id))
}
